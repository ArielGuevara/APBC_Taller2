...
while True:
    ...
    if opcion == '1':
        ...
        if res.get("status") == "ok":
            data = res.get("data", {})
            print(f"Nombre: {data.get('Nombre')}, Materia: {data.get('Materia')}, Calificacion: {data.get('Calificacion')}")
        else:
            print(res.get('mensaje', 'Error desconocido'))
    elif opcion == '3':
        id_est = input("ID: ")
